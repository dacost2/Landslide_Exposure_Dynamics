...
# %%
# Testing-only cell: run this after the main workflow has produced master_grid.
if "master_grid" not in globals():
    raise RuntimeError("master_grid is not in memory. Run the main workflow cell first.")

threshold = 10
led_col = "LED_count"
hisdac_col = "BUPR_value"

required_cols = {"HISDAC_id", led_col, hisdac_col, "geometry"}
missing_cols = sorted(required_cols.difference(master_grid.columns))
if missing_cols:
    raise ValueError(f"Missing required columns in master_grid: {missing_cols}")

error_cells = master_grid[["HISDAC_id", led_col, hisdac_col, "geometry"]].copy()
error_cells = error_cells.replace([np.inf, -np.inf], np.nan).dropna(subset=[led_col, hisdac_col])
error_cells["error_diff"] = error_cells[led_col].astype(float) - error_cells[hisdac_col].astype(float)
error_cells["abs_error"] = error_cells["error_diff"].abs()

large_error_cells = error_cells[error_cells["abs_error"] > threshold].copy()
print(
    f"Large-error cells (|{led_col} - {hisdac_col}| > {threshold}): "
    f"{len(large_error_cells)}/{len(error_cells)} "
    f"({(100 * len(large_error_cells) / len(error_cells)):.2f}%)"
)

fig, axes = plt.subplots(1, 2, figsize=(14, 6))

# Left panel: location of flagged cells.
error_cells.boundary.plot(ax=axes[0], linewidth=0.05, color="lightgray")
if "wa_boundary" in globals() and wa_boundary is not None and not wa_boundary.empty:
    wa_boundary.to_crs(error_cells.crs).boundary.plot(ax=axes[0], linewidth=0.6, color="black")
if not large_error_cells.empty:
    large_error_cells.plot(ax=axes[0], color="crimson", edgecolor="none", alpha=0.8)
axes[0].set_title(
    f"{globals().get('state_code', 'STATE')} {globals().get('analysis_year', 'YEAR')}: "
    f"|{led_col} - {hisdac_col}| > {threshold}"
)
axes[0].set_axis_off()

# Right panel: signed error among flagged cells.
if not large_error_cells.empty:
    vmax = max(1.0, float(np.nanmax(np.abs(large_error_cells["error_diff"].to_numpy()))))
    large_error_cells.plot(
        ax=axes[1],
        column="error_diff",
        cmap="coolwarm",
        vmin=-vmax,
    ...
    axes[1].text(0.5, 0.5, "No cells exceed threshold", ha="center", va="center")

if "wa_boundary" in globals() and wa_boundary is not None and not wa_boundary.empty:
    wa_boundary.to_crs(error_cells.crs).boundary.plot(ax=axes[1], linewidth=0.6, color="black")
axes[1].set_title("Signed error in flagged cells (LED_count - BUPR_value)")
axes[1].set_axis_off()
...
# %%
# Testing-only cell: aggregate LED sqft to grid and compare with BUI_mean.
if "master_grid" not in globals() or "led_with_id" not in globals():
    raise RuntimeError("master_grid and led_with_id must be in memory. Run the main workflow cell first.")

if "sqft" not in led_with_id.columns:
    raise ValueError("led_with_id is missing required column: sqft")
if "HISDAC_id" not in led_with_id.columns:
    raise ValueError("led_with_id is missing required column: HISDAC_id")
if "BUI_mean" not in master_grid.columns:
    raise ValueError("master_grid is missing required column: BUI_mean")

sqft_numeric = pd.to_numeric(led_with_id["sqft"], errors="coerce")
sqft_by_id = (
    pd.DataFrame({"HISDAC_id": led_with_id["HISDAC_id"], "sqft": sqft_numeric})
    .dropna(subset=["HISDAC_id", "sqft"])
    .groupby("HISDAC_id", as_index=True)["sqft"]
    .sum()
)

master_grid["LED_sqft_sum"] = master_grid["HISDAC_id"].map(sqft_by_id).fillna(0.0)

sqft_eval = master_grid[["HISDAC_id", "LED_sqft_sum", "BUI_mean", "geometry"]].copy()
sqft_eval = sqft_eval.replace([np.inf, -np.inf], np.nan).dropna(subset=["LED_sqft_sum", "BUI_mean"])

if sqft_eval.empty:
    raise ValueError("No valid rows found for LED_sqft_sum vs BUI_mean evaluation.")

y_true = sqft_eval["BUI_mean"].astype(float).to_numpy()
y_pred = sqft_eval["LED_sqft_sum"].astype(float).to_numpy()
diff = y_pred - y_true

sqft_metrics = {
    "n_cells": int(len(sqft_eval)),
    "correlation": float(np.corrcoef(y_true, y_pred)[0, 1]) if len(sqft_eval) > 1 else np.nan,
    "rmse": float(np.sqrt(np.mean(diff**2))),
    "mae": float(np.mean(np.abs(diff))),
    "bias": float(np.mean(diff)),
    "mean_led_sqft_sum": float(np.mean(y_pred)),
    "mean_bui_mean": float(np.mean(y_true)),
}

print("\nEvaluation Metrics (LED_sqft_sum vs BUI_mean):")
for k, v in sqft_metrics.items():
    if k == "n_cells":
        print(f"- {k}: {v}")
    else:
        print(f"- {k}: {v:.4f}")
...
max_val = float(max(np.nanmax(y_true), np.nanmax(y_pred)))
axes[0].plot([min_val, max_val], [min_val, max_val], "k--", linewidth=1)
axes[0].set_xlabel("BUI_mean")
axes[0].set_ylabel("LED_sqft_sum")
axes[0].set_title("LED_sqft_sum vs BUI_mean")

axes[1].hist(diff, bins=40, color="steelblue", alpha=0.8)
axes[1].axvline(0, color="black", linestyle="--", linewidth=1)
axes[1].set_xlabel("Difference (LED_sqft_sum - BUI_mean)")
axes[1].set_ylabel("Grid cell count")
axes[1].set_title("Distribution of Differences")
...
sqft_eval["error_diff"] = diff
sqft_eval["abs_error"] = sqft_eval["error_diff"].abs()
sqft_threshold = 10
sqft_large_error = sqft_eval[sqft_eval["abs_error"] > sqft_threshold].copy()

print(
    f"Large-error cells (|LED_sqft_sum - BUI_mean| > {sqft_threshold}): "
    f"{len(sqft_large_error)}/{len(sqft_eval)} "
    f"({(100 * len(sqft_large_error) / len(sqft_eval)):.2f}%)"
)

fig, axes = plt.subplots(1, 2, figsize=(14, 6))
sqft_eval.boundary.plot(ax=axes[0], linewidth=0.05, color="lightgray")
if "wa_boundary" in globals() and wa_boundary is not None and not wa_boundary.empty:
    wa_boundary.to_crs(sqft_eval.crs).boundary.plot(ax=axes[0], linewidth=0.6, color="black")
if not sqft_large_error.empty:
    sqft_large_error.plot(ax=axes[0], color="crimson", edgecolor="none", alpha=0.8)
axes[0].set_title("Large absolute errors: |LED_sqft_sum - BUI_mean| > 10")
axes[0].set_axis_off()

if not sqft_large_error.empty:
    vmax = max(1.0, float(np.nanmax(np.abs(sqft_large_error["error_diff"].to_numpy()))))
    sqft_large_error.plot(
        ax=axes[1],
        column="error_diff",
        cmap="coolwarm",
        vmin=-vmax,
...
if "wa_boundary" in globals() and wa_boundary is not None and not wa_boundary.empty:
    wa_boundary.to_crs(sqft_eval.crs).boundary.plot(ax=axes[1], linewidth=0.6, color="black")
axes[1].set_title("Signed error in flagged cells (LED_sqft_sum - BUI_mean)")
axes[1].set_axis_off()
